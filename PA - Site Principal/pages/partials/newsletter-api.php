    ...
    try {
        ...
        if (isset($response['body']['newsletters']) && is_array($response['body']['newsletters'])) {
            foreach ($response['body']['newsletters'] as &$newsletter) {
                ...
                $newsletter['created_by_user_type_label'] = getSenderTypeLabel($newsletter['created_by_user_type'] ?? 0);
            }
        }
...
function forwardCreateNewsletter($apiBase, $jwtToken, $jsonData = null) {
    ...
    $payload = [
        'title' => $title,
        'content' => $content,
        'created_by_user_type' => strval($user['user_type'] ?? 0)
    ];
...
function getSenderTypeLabel($senderType) {
    ...
    return $labels[intval($senderType)] ?? 'Unknown sender';
}
?>