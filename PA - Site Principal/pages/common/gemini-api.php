...
if ($result === false || $curlErr !== '') {
    ...
    http_response_code(502);
    echo json_encode(['error' => 'Failed to reach Gemini API', 'debug' => $curlErr]);
    exit;
}

error_log("[gemini-api] Response ($httpCode): " . substr($result, 0, 2000));

$geminiData = json_decode($result, true);

if ($httpCode !== 200) {
    error_log("[gemini-api] Non-200 response ($httpCode): " . substr($result, 0, 500));
...
if (!is_array($geminiData)) {
    error_log("[gemini-api] Invalid JSON response: " . json_last_error_msg());
    http_response_code(502);
...
if (!isset($geminiData['candidates'][0]['content']['parts'][0]['text'])) {
    error_log("[gemini-api] Unexpected response structure: " . json_encode($geminiData));
    http_response_code(502);
