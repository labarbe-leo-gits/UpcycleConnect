(function() {
    ...
    function formatDateTime(value) {
        if (!value) return '';
        var datePart = value.split(' ')[0] || value;
        var timePart = (value.split(' ')[1] || '').slice(0,5);
    ...
    if (scheduleCardsContainer) {
        ...
        var setSelectedSchedule = function(scheduleId) {
            ...
            scheduleConflictConfirmed = false;
            closeConflictModal();
            console.log('setSelectedSchedule', scheduleId);
        ...
        cards.forEach(function(card) {
            card.addEventListener('click', function() {
                console.log('schedule card clicked', card.dataset.scheduleId);
            ...
            if (result.paymentIntent && result.paymentIntent.status === 'succeeded') {
                ...
                if (verifyResponse.ok && verifyData && verifyData.status === 'succeeded') {
                    ...
                    if (scheduleSelect) {
                        ...
                        var selectedCard = document.querySelector('#schedule-cards .schedule-card.selected');
                        if (selectedCard && selectedCard.dataset.scheduleId) {
                            scheduleId = selectedCard.dataset.scheduleId;
                        }
                    }
                    if (!scheduleId) {
                        var hiddenFree = document.getElementById('event_availability_id_free');
                        var hiddenPaid = document.getElementById('event_availability_id_paid');
                        scheduleId = (hiddenPaid && hiddenPaid.value) || (hiddenFree && hiddenFree.value) || '';
                    }
