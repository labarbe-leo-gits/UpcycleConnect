...
func GetInvoicesByUserID(w http.ResponseWriter, r *http.Request) {
	...
	idStr = strings.TrimSuffix(idStr, "/invoices")
	...
	invoices, err := db.GetInvoicesByUserID(userID)
	if err != nil {
		fmt.Println("[ERROR] GetInvoicesByUserID:", err)
		sendError(w, "Unable to fetch invoices", http.StatusInternalServerError)
		return
	}
